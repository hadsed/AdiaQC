...
//TODO: if needed, these can easily be embedded in the simulation loop
void scaleVec( double complex *vec, const double s, const uint64_t N ){
    uint64_t i;
...
void expMatTimesVec( double complex *vec, const double *mat, const double complex cc, const uint64_t N ){
    uint64_t i;
...
void findLargest( uint64_t *listN, const double complex *list, const uint64_t size, const uint64_t N ){
    uint64_t i;
    double temp;
