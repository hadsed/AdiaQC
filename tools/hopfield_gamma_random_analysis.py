'''
File: hopfield_gamma_random_analysis.py
Author: Hadayat Seddiqi
Date: 5.13.14
Description: Do energy gap plots for varying Gammas
             of random instances with N qubits (including
             different numbers of patterns) in an attempt
             to tune Gamma for large scale simulations.
'''
...
if __name__=="__main__":
    ...
    nsamples = 10000
    samples = np.random.random_integers(0,9999,nsamples)
    data = []
    ...
    # Record the average Gamma value for successful instances
    avg = {'hebb': 0.0, 'stork': 0.0, 'proj': 0.0}
    avgcount = {'hebb': 0, 'stork': 0, 'proj': 0}
    ...
    for root in samples:
        ...
        # ax1.plot(times, eigs[:,1]-eigs[:,0], c=color, linewidth=lwidth, alpha=0.5)
        # ax1_cb = fig.add_axes([0.05, 0.08, 0.29, 0.01])
        # ax1_cb_b = mpl.colorbar.ColorbarBase(ax1_cb, cmap=c1, 
    ...
    for root in samples:
        ...
        # ax2.plot(times, eigs[:,1]-eigs[:,0], c=color, linewidth=lwidth, alpha=0.5)
        # ax2_cb = fig.add_axes([0.355, 0.08, 0.29, 0.01])
        # ax2_cb_b = mpl.colorbar.ColorbarBase(ax2_cb, cmap=c2, 
        #                                      orientation='horizontal')
    ...
    for root in samples:
        ...
        # ax3.plot(times, eigs[:,1]-eigs[:,0], c=color, linewidth=lwidth, alpha=0.5)
        # ax3_cb = fig.add_axes([0.66, 0.08, 0.29, 0.01])
        # ax3_cb_b = mpl.colorbar.ColorbarBase(ax3_cb, cmap=c3, 
        #                                      orientation='horizontal')
        # print success, sprob, gamma
        os.chdir('../')
    os.chdir('../')
    # pl.savefig('gamma_tuning_'+str(nsamples)+'.png')
    for k,v in avgcount.iteritems():
        avg[k] /= v
    print "Average values for Gamma:"
    print avg
